...
trainset = [
    dspy.Example(
        text="The stock market rallied on strong earnings reports", category="finance"
    ).with_inputs("text"),
    dspy.Example(
        text="New study reveals high-protein diets improve recovery", category="health"
    ).with_inputs("text"),
    dspy.Example(
        text="SpaceX successfully launched 23 Starlink satellites", category="technology"
    ).with_inputs("text"),
    dspy.Example(
        text="Team wins championship after dramatic overtime victory", category="sports"
    ).with_inputs("text"),
...
# Step 9: Evaluate
devset = [
    dspy.Example(text="NASA announced a new mission to Mars", category="technology").with_inputs(
        "text"
    ),
    dspy.Example(
        text="Gold prices reached an all-time high amid inflation fears", category="finance"
    ).with_inputs("text"),
]

evaluator = Evaluate(
    devset=devset, metric=classification_metric, num_threads=4, display_progress=True
)
