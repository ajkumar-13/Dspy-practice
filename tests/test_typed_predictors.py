...
class TestExtractedEntity:
    def test_valid_person(self):
        entity = ExtractedEntity(
            name="Tim Cook", entity_type="person", confidence=0.95
        )
    ...
    def test_valid_organization(self):
        entity = ExtractedEntity(
            name="Apple Inc.", entity_type="organization", confidence=0.99
        )
        assert entity.entity_type == "organization"

    def test_valid_location(self):
        entity = ExtractedEntity(
            name="Cupertino", entity_type="location", confidence=0.85
        )
        assert entity.entity_type == "location"

    def test_valid_date(self):
        entity = ExtractedEntity(
            name="January 25, 2025", entity_type="date", confidence=0.92
        )
        assert entity.entity_type == "date"

    def test_valid_amount(self):
        entity = ExtractedEntity(
            name="$94.8 billion", entity_type="amount", confidence=0.88
        )
    ...
    def test_json_roundtrip(self):
        entity = ExtractedEntity(
            name="Paris", entity_type="location", confidence=0.9
        )
